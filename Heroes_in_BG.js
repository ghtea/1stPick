...
function updatePage (){
    ...
    for (var i = 0; i<88; i++) {
        dataMap[i]['Point'] = (100-ratio) * (dataMap[i]['WinRate'] /50 / stdWinRate ) 
            + ratio * (   (dataMap[i]['PlayRate'] + dataMap[i]['BanRate'] ) / 100 * 88 / 16 / stdGame );
    };
    ...
    for (var i = 0; i<numShowingHero; i++){
        ...
        Rect.style = "width:" + RectWidth + "px;height:" +  RectHeight + "px; background: linear-gradient(200deg, rgba(105,245,168,1) 0%, rgba(17,226,97,1) 40%, rgba(17,226,97,1) 100%); ";
